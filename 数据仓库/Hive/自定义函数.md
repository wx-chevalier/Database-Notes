...
•直接 select 中使用：select explode_map(properties) as (col1,col2) from src;
...
lateral view
• Lateral View 语法
•lateralView: LATERAL VIEW udtf(expression) tableAlias AS columnAlias (',' columnAlias)_ fromClause: FROM baseTable (lateralView)_
